...
cluster: B
tags: [MOC, 이차형식, Cochran, HatMatrix, ANOVA, 무작위화, 연구설계, ClusterB, Hub]
...
### ⑥ 전제 — SS 분해가 '의미'를 가지려면 (2026-08-22 신규 가지)
> 가지 ①~⑤가 **계산의 대수**라면, 이 가지는 그 계산에 **해석의 자격**을 부여하는 조건을 다룬다.
- [[자기참조적 집단화와 F검정의 함정]] — 반응변수로 집단을 만들면 효과가 없어도 기각률 **100%**. $\beta_1=2\sigma\sqrt{2/\pi}$ 가 전부 내생성 편의($\operatorname{Cov}(g,\varepsilon)=\sigma\phi(0)$), $R^2=2/\pi$. **어떤 진단도 잡지 못한다** (Cluster B∩C 매개 노드)
- [[무작위화 (Randomization)]] — Montgomery §1.3. 귀무분포 $F(k-1,N-k)$ 의 **자격증**이 어디서 오는가. 동전던지기가 사주는 5%
- [[표본분포 (Sampling Distribution)]] — 뿌리. 모집단분포 / 표본 / 통계량의 분포 3층 구분, $t_\nu=Z/\sqrt{W/\nu}$ 의 자유도가 곧 차원
- [[내생성 (Endogeneity)]] · [[데이터 누출 (Data Leakage)]] · [[절단정규분포 (Truncated Normal)]] — 주변 개념 (stub)
- 진단 쪽 짝: [[Levene 검정]] · [[등분산성(homoscedasticity)]] — **통과해도 타당성을 보증하지 않는다**는 반례를 이 가지가 제공
- 📄 재현 코드·그림: `과목별/실험설계법/ANOVA_자기참조집단화_시뮬레이션/`
...
>
> **0단계(2026-08-22 추가)**: 그 전에 [[표본분포 (Sampling Distribution)]] 로 "통계량 하나의 분포"를 세우고, [[자기참조적 집단화와 F검정의 함정]] 으로 **이 분해가 언제 의미를 잃는지**를 먼저 본다. 대수보다 전제가 먼저다.
...
## 🔁 이어가기 — 2026-08-22 세션
1. **대상**: `[[자기참조적 집단화와 F검정의 함정]]` · `[[무작위화 (Randomization)]]` · `[[표본분포 (Sampling Distribution)]]` (신규 원자노트 3) · **가지 ⑥ 신설** · 트랙 A
2. **오늘 한 것**: "ANOVA를 새로운 관점에서" — 학습자의 $t_{99}$ 오해에서 출발해 표본분포 3층 구분 교정 → 학습자가 제안한 $X>50$ 집단화를 끝까지 밀어 **F검정의 전제조건**을 역으로 조명. 시뮬레이션 5만 회 전수 검증. 빈 stub였던 `[[Levene 검정]]`·`[[등분산성(homoscedasticity)]]` 본문 작성
3. **핵심 상태**: 가지 ⑥ 완결. $\beta_1=2\sigma\sqrt{2/\pi}=15.958$ (이론=시뮬 소수 3자리 일치), $R^2=2/\pi$, 기각률 100%, Levene 기각률 5.17%(무력). **미완**: "$F\sim F(1,98)$ 의 필요조건" 을 학습자가 한 줄로 적는 단계에서 중단
4. **다음 출발점** →: **[[이차형식]] §3.4 (p114–119)**. 오늘 "자유도 = 부분공간의 차원"이라는 동기가 확보됨. $P=\frac1n\mathbf{11}^\top$ 의 대칭·멱등 → $\operatorname{tr}(I-P)=99$ 부터 시작
5. **연결**: `[[Ch03_선형모형의 확률구조 — 고정설계·모수와 추정량·σ²I]]` · `[[내생성 (Endogeneity)]]` · `[[누락변수 편의와 유의성 반전]]`
